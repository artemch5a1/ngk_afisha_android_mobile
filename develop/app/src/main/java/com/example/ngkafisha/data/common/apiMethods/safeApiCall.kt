...
import com.example.ngkafisha.domain.common.exceptions.ApiErrorMessages.getMessage
...
suspend fun <T> safeApiCall(call: suspend () -> T): T {
    return try {
        ...
        throw ApiException(-1, "Сервер недоступен. Проверьте подключение к интернету и повторите снова")
    }
    catch (ex: Exception) {
